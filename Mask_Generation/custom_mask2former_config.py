...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        ann_file=
        'custom_dataset/train/coco_json_combined_dataset_size_512_coco_dataset_faulty_image_removed_category_id_changed.json',
        img_prefix='custom_dataset/train/Images',
        pipeline=[
            dict(type='LoadImageFromFile', to_float32=True),
            dict(type='LoadAnnotations', with_bbox=True, with_mask=True),
            dict(type='RandomFlip', flip_ratio=0.5),
    ...
    val=dict(
        type='CocoDataset',
        ann_file=
        'custom_dataset/val/coco_json_Validation_Combined_Dataset_Size_512_coco_dataset_category_id_changed.json',
        img_prefix='custom_dataset/val/Original',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
    ...
    test=dict(
        ...
        classes=('text', )))
evaluation = dict(
    interval=5000, metric=['bbox', 'segm'], dynamic_intervals=[(36500, 36875)])
...
lr_config = dict(
    ...
    warmup_iters=10)
max_iters = 36875
runner = dict(type='IterBasedRunner', max_iters=36875)
interval = 5000
dynamic_intervals = [(36500, 36875)]
